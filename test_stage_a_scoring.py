...
class ScoringTests(unittest.TestCase):
    ...
    def test_result_content_model_and_user_language(self):
        ...
        footnote=presentation["resultDetails"]["actionFootnote"]
        self.assertTrue(isinstance(footnote,str) or set(footnote)=={"ongoing","ended"})
        for report in DATA["reports"].values():
            self.assertTrue(report["share"])
            self.assertNotIn("更值得看的",report["share"])
            self.assertNotIn("可以先",report["share"])

    def test_insight_rules_are_multi_question_patterns(self):
        rules=DATA["resultPresentation"]["insightRules"]
        self.assertGreaterEqual(len(rules),3)
        for rule in rules:
            evidence_questions={question for group in rule["evidenceGroups"] for question in group["questions"]}
            self.assertGreaterEqual(len(evidence_questions),2,rule["id"])
            self.assertTrue(rule["body"])
            self.assertTrue(rule["actions"])
